...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#اعاده_التشغيل \n" "⪼ بوت ستارك في وضع اعاده التشغيل انتظر"
        )
    await edit_or_reply(
        event,
        f"**⌔∮ اهلا عزيزي** - {mention}\n"
        f"**يتم الان اعـادة تشغيـل بـوت ستارك يستغـرق الامر 1-2 دقيقه ▬▭ ...**",
    )
...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#اطفاء \n" "بوت ستارك في وضع الاطفاء"
        )
    await edit_or_reply(
        event, "**جارٍ إيقاف تشغيل بوت ستارك الآن ... شغِّلني يدويًا لاحقًا**"
    )
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **╮•❐ الامـر ⦂ **`.ايقاف`\
        \n**  •  الشـرح •• **__لإيقاف تشغيل البوت من هيروكو. لا يمكنك اعادة التشغيل بواسطة الروبوت ، فأنت بحاجة إلى الوصول إلى هيروكو وتشغيله أو استخدام ___ @XRXNR"
    }
)
