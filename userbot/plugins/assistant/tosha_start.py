...
# =================== OWNER - XRXNR =================== #
ALIVE_NAME = Config.ALIVE_NAME if Config.ALIVE_NAME else "@XRXNR"
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        "logs.txt",
        reply_to=event.id,
        caption="**Heroku** STARK Logs",
        buttons=[
            [Button.url("💠 𝙑𝙄𝙀𝙒 𝙊𝙉𝙇𝙄𝙉𝙀 💠", f"{url}")],
            [Button.url("💠 𝘾𝙍𝘼𝙎𝙃𝙀𝘿 💠", "t.me/XRXNR")]
        ])
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"STARK"))
          )  # pylint: disable=oof
async def settings(event):
    await event.delete()
...
async def settings(event):
    await event.edit("** ⌔∮ هنا ستجد رابط زد ثـون ورابط التنصيب ايضا**:",
                     buttons=[
                         [Button.url("🔗 رابط السورس🔗", url="https://github.com/STARKthon/ZED_USERBOT")],
                         [Button.url("🔗 رابط التنصيب 🔗", url="https://t.me/XRXNR/105")],
                         [Button.url("✨ الدعم ✨",
                                     url="https://t.me/XRXNR")]
                     ])
...
async def settings(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @XRXNR", alert=True)


# STARK zag - زغرفه ستارك
